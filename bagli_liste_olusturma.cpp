...
int main()
{
	...
	while(geciciDugumTek!=NULL)
	{
		...
		tekDizi[sayacTek]=a; //oratncayi bulmak icin bagli listeyi donguye aldim
		printf(" %d,",a);
		if(a>enBuyukTek)   //dizinin en buyuk elemanýný bulmak icin
		{
			enBuyukTek=a;
		}
		if(a <enKucukTek)
	...
	geciciDugumTek=ilkDugumTek;
	geciciDugumCift=ilkDugumCift;
    while(geciciDugumTek!=NULL ) //once tek sayilardan olusan bagli lisyeyi yeni bri bagli listeye aldim
	{
	 ...
	 while(geciciDugumCift!=NULL ) //sonra bu yeni bagli listenin üzerine cift sayialri ekledim
	{
	...
	for(int i=0;i<N;i++) //listeyi kucukten buyuge siralamak icin
    {
    	while(1) //while(geciciDugum!=NULL) yazdigim zaman donguden cýkmýyordu,cýkmasi icin kontrolu dongunun icinde yaptým
    	{
	    	Dugum*yeniDugum=geciciDugum->sonraki;
		    if(geciciDugum->sayi > yeniDugum->sayi) //dugumun sayi degeri  tuttugu adresteki sayi degerinden daha buyukse yerlerini degistirmek icin
	    	{
